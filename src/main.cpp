...
#if defined(NDEBUG)
#error "NulleX cannot be compiled without assertions."
#endif
...
// Don't sync this code
    if (nHeight == 0) {
        ...
        nSubsidy = 8 * COIN;    
    } else if (nHeight >= 600001 && nHeight <= 750000) {
        nSubsidy = 6 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 1000) {
        ret = 0;
    } else if (nHeight >= 1001 && nHeight < 20001) {
        ret = blockValue * .50;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NulleX
...
void RecalculateZICHSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNLX supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zNLX money supply in the block index
    if (!UpdateZICHSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zNLX supply for block=%s height=%d", __func__,
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NulleX: We use certain sporks during IBD, so check to see if they are
