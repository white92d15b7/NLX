...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NLX);
    ...
    if (!settings.contains("nAnonymizeNulleXAmount"))
        settings.setValue("nAnonymizeNulleXAmount", 1000);

    nAnonymizeNulleXAmount = settings.value("nAnonymizeNulleXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNulleXAmount"))
        SoftSetArg("-anonymizeNulleXamount", settings.value("nAnonymizeNulleXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in NulleX.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNulleXAmount:
            return QVariant(nAnonymizeNulleXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNulleXAmount:
            nAnonymizeNulleXAmount = value.toInt();
            settings.setValue("nAnonymizeNulleXAmount", nAnonymizeNulleXAmount);
            emit anonymizeNulleXAmountChanged(nAnonymizeNulleXAmount);
            break;
        case CoinControlFeatures:
